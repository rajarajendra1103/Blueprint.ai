...
const SECTION_ORDER: SpecSectionId[] = [
  'architecture',
  'requirements',
  'algorithms',
  'dataModel',
...
export function synthesizeFallbackSectionContent(sectionId: SpecSectionId, idea: string, classification: ClassificationResult): string {
...
export async function generateFullSpec(
  ...
  options: { hybrid?: boolean } = { hybrid: true }
): Promise<SpecDoc> {
  ...
  const foundationSections: SpecSectionId[] = ['architecture', 'requirements', 'dataModel'];
  ...
  for (const sectionId of foundationSections) {
    ...
    try {
      content = await withTimeout(
        adapter.generate(prompt, config.apiKey, config.model, {
          systemPrompt,
          temperature: 0.5,
        }),
        65000,
