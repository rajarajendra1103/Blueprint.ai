import { ConsistencyWarning, SpecSection, SpecSectionId } from '@blueprint/shared';

export function checkSpecConsistency(sections: Record<SpecSectionId, SpecSection>): ConsistencyWarning[] {
  ...
  const discoveredRoutes: { method: string; path: string }[] = [];
  ...
  if (discoveredRoutes.length > 0 && folderStructureContent) {
    const hasRoutesOrControllers = 
      folderStructureContent.toLowerCase().includes('routes') ||
