...
export function getDomainAdaptivePresets(classification: ClassificationResult): [DesignDirection, DesignDirection] {
  ...
  if (domain.includes('developer') || domain.includes('tool') || domain.includes('security') || domain.includes('cloud')) {
    return [COMPATIBILITY_PRESETS.cyberMonoline, COMPATIBILITY_PRESETS.minimalistSlate];
  }
  if (domain.includes('finance') || domain.includes('fintech') || domain.includes('banking') || domain.includes('enterprise')) {
    return [COMPATIBILITY_PRESETS.swissLuxe, COMPATIBILITY_PRESETS.minimalistSlate];
  }
  if (domain.includes('health') || domain.includes('medical') || domain.includes('wellness')) {
    return [COMPATIBILITY_PRESETS.clinicalClarity, COMPATIBILITY_PRESETS.minimalistSlate];
  }
...
export async function generateDesignDirections(
  ...
  const { prompt, systemPrompt } = buildDesignPrompt(idea, classification);
