...
export class OpenRouterAdapter implements LLMProvider {
  ...
  async generate(prompt: string, apiKey: string, model: string = 'openrouter/free', options?: GenerateOptions): Promise<string> {
    ...
    const isExhaustedOrRateLimited = (e: any) => {
    ...
    try {
      ...
      if (isFreeModel && isExhaustedOrRateLimited(err)) {
        ...
        const friendlyError: any = new Error(
          `Free tier limit reached for model '${model}' on OpenRouter. The free requests / rate limits for this model have been exceeded. Please click "Change Model" in Provider Settings to select another free model or switch to Google Gemini.`
        );
